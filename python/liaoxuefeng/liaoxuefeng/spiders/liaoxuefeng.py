...
class LiaoxuefengSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        # 这是廖雪峰老师python教程的标题列表。我们利用xpath解析器对收到的response进行分析，从而提取出我们需要的数据。
        # //XXX表示任何任何目录下的XXX区块，/XXX表示子目录下的XXX区块，XXX[@class=abc]表示带有class=abc属性值的XXX区块，/text()表示获取该区块的文本。
        # 最后加上.extract()表示将内容提取出来。
        for title in titles:
            print (title)
